...
const getChannelStats = asyncHandler(async (req, res) => {
        ...
        pipeline: [
          {
            $match: {
              $expr: {
                $and: [{ $in: ['$postId', '$$videoIds'] }, { $eq: ['$mediaType', 'Video'] }],
              },
            },
          },
        ],
        ...
        pipeline: [
          {
            $match: {
              $expr: {
                $and: [{ $in: ['$postId', '$$tweetId'] }, { $eq: ['$mediaType', 'Tweet'] }],
              },
            },
          },
        ],
  ...
  return res.status(200).json(new ApiResponse(200, stats, 'user stats are fetched successfully'))
})
