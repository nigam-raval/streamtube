...
const registerUser = asyncHandler(async (req, res) => {
  ...
  if (coverContentLength) {
    // any cover field can used as condition , just want to check if user want to upload cover image?
  ...
  const avatarUploadUrl = await generatePresignedUploadUrl(
    avatarContentType,
    avatarContentLength,
    avatarChecksumSHA256,
    avatarKey
  ...
  if (coverContentLength) {
    ...
    coverUploadUrl = await generatePresignedUploadUrl(
      coverContentType,
      coverContentLength,
      coverChecksumSHA256,
      coverKey
  ...
  //return res
  return res.status(201).json(new ApiResponse(200, response, 'User registered successfully'))
})
    ...
    return res
      ...
      .json(
        new ApiResponse(
          ...
          'Access token and Refresh token are refreshed'
        )
      )
  } catch (error) {
...
const getCurrentUser = asyncHandler(async (req, res) => {
  ...
  return res.status(200).json(new ApiResponse(200, response, 'current user fetched successfully'))
})
...
const updateUserCoverImage = asyncHandler(async (req, res) => {
  ...
  const coverUploadUrl = await generatePresignedUploadUrl(
    coverContentType,
    coverContentLength,
    coverChecksumSHA256,
    coverKey
...
const deleteUser = asyncHandler(async (req, res) => {
  ...
  return res.status(200).json(new ApiResponse(200, {}, `username: ${user.username} is deleted`))
})
